...
def run_command(cmd_args, evaluate=True):
  ...
  return eval(out) if evaluate else out
...
def print_result(address, run_mode, additional_msg=None, do_print=True):
  ...
  string = (((((((
      ((((((f"{BLACK}Access with " + RED) + "%s") + BLACK) + " on ") + BLUE) +
       "0x%016x") + BLACK) + ": took ") + MAGENTA) + "%d ") + BLACK) +
             "cycles")) % (mode, addr, cycle)
...
def measure_address(address, status_str, mode_str, priv_str, add_str):
  print(NORMAL)
  print(f"Measuring timing for {status_str} address ({mode_str} access)")
  print("type ';' with newline to move on to next measure")
  print(BOLD)
  print((((((f"{BLACK}Target address: {BOLD}{BLUE}{address}" + GREEN) + " <- ")
           + RED) + priv_str) + BLACK))
  print("")
  ...
  a = ""
  while not a:
    mode = 'readmem' if mode_str == 'read' else 'jmp'
...
if __name__ == '__main__':
  ...
  measure_address("0xffffffff00000000", f"{RED}Unmapped{NORMAL}", "read",
                  "unmapped", U_str)
  ...
  print(BLUE + out + BLACK)
    ...
    # measure mapped (read)
  measure_address(mapped_address, f"{BLUE}Mapped{NORMAL}", "read", "mapped",
                  M_str)
  ...
  measure_address("0xffffffff00000000", f"{RED}Unmapped{NORMAL}", "exec",
                  "unmapped", U_str)
    # measure mapped but non-executable (exec)
  measure_address(
      mapped_address,
      f"{MAGENTA}Mapped, but Non-executable{NORMAL}",
      "exec",
      "non-executable",
      NX_str,
  )
  ...
  measure_address(
      exec_address,
      f"{BLUE}Mapped and Executable{NORMAL}",
      "exec",
      "executable",
      X_str,
  )
